...
async fn main() {
    ...
    if let Err(e) = lambda_http::run(service_fn(|event| async {
        handler(event, &api_gw_client, &dynamo_client, &http_client).await
    }))
    .await
    {
...
async fn handler(
    ...
    match route_key.as_str() {
        ...
        "$disconnect" => {
            ...
            handler::disconnect::ws_disconnect(connection_id, http_client.clone()).await?;
            return Ok(Response::builder()
                .status(200)
                .body(Body::Text(json!({ "status": "disconnect" }).to_string()))?);
        }
            ...
            match serde_json::from_slice::<WsRequestMessage>(&body) {
                Ok(body) => match body {
                    ...
                    WsRequestMessage::TopicSubscribe { topic } => {
                        handler::default::topic_subscribe(
                            connection_id,
                            http_client.clone(),
                            &topic,
                        )
                    ...
                    WsRequestMessage::TopicUnSubscribe { topic } => {
                        handler::default::topic_unsubscribe(
                            connection_id,
                            http_client.clone(),
                            &topic,
                        )
